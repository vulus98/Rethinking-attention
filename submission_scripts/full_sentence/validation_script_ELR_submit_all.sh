...
if [ $# == 0 ]; then
    echo ""
    echo "submission_scripts/full_sentence/validation_script_ELR_submit_all.sh <architecture_name> [--decoder]?"
    echo
    echo "Args:"
...
sbatch  --output=./sbatch_log/ELR/$1/evaluation_outputs/%j.out submission_scripts/full_sentence/validation_script.sh --substitute_model_path$suffix $SCRATCH/pytorch-original-transformer/models/checkpoints/ELR/$1/  --epoch$suffix $epoch --substitute_type$suffix ELR --substitute_class$suffix $1 
