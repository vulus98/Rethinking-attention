...
from models.definitions.transformer_model import mha_to_mha3
"""
B = batch size
S = sentence length
MD = model dimension
...
Extracted ouptut shape: B x NH x S x S

For each sentence in the batch and for each head a matrix SxS of attention weights is extracted.
"""
def extract_input_output(training_config):
    ...
    for f in os.listdir(ATTENTION_WEIGHTS_OUTPUT_PATH):
        full_name = f"{ATTENTION_WEIGHTS_OUTPUT_PATH}/{f}"
    ...
    def getf(i, suffix):
        def write_input_output(model, input, output):
            ...
            in_filename_q = f"{ATTENTION_WEIGHTS_OUTPUT_PATH}/{prefix}_layer{i}_q_inputs_{suffix}"
            in_filename_k = f"{ATTENTION_WEIGHTS_OUTPUT_PATH}/{prefix}_layer{i}_k_inputs_{suffix}"
            in_filename_v = f"{ATTENTION_WEIGHTS_OUTPUT_PATH}/{prefix}_layer{i}_v_inputs_{suffix}"
            
            out_filename = f"{ATTENTION_WEIGHTS_OUTPUT_PATH}/{prefix}_layer{i}_outputs_{suffix}"
    ...
    def extract(token_ids_loader, suffix):
        ...
        for (i, l) in enumerate(transformer.encoder.encoder_layers):
            h = l.multi_headed_attention.attention_w.register_forward_hook(getf(i, suffix))
            hook_handles.append(h)
        mask_filename = f"{ATTENTION_WEIGHTS_OUTPUT_PATH}/{prefix}_masks_{suffix}"
